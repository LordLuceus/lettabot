...
import type { SendMessageResponse, ChatResponse, AsyncChatResponse, PairingListResponse, PairingApproveRequest, PairingApproveResponse } from './types.js';
...
import { loadConfigStrict, saveConfig } from '../config/io.js';
import type { LettaBotConfig } from '../config/types.js';
import { CONFIG_SCHEMA } from '../config/schema.js';
import { maskSensitiveFields, deepMergeConfig, needsRestart } from '../config/mutations.js';
import { servePortalFile } from './portal.js';
import { TurnViewer } from './turn-viewer.js';
import {
  VALID_CHANNELS, MAX_BODY_SIZE, MAX_TEXT_LENGTH, MAX_FILE_SIZE, WEBHOOK_CONTEXT,
  readBody, sendError, parseWebhookChatRequest,
...
export function createApiServer(deliverer: AgentRouter, options: ServerOptions): http.Server {
  const turnViewer = new TurnViewer(options.turnLogFiles);
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (options.turnLogFiles && req.method === 'GET') {
      ...
      if (parsedUrl.pathname === '/turns/data') {
        ...
        res.end(JSON.stringify(await turnViewer.readTurns(filePath)));
        return;
      }
      if (parsedUrl.pathname === '/turns/stream') {
        ...
        const allTurns = await turnViewer.readTurns(filePath);
        res.write(`data: ${JSON.stringify({ type: 'init', turns: allTurns })}\n\n`);
        const removeClient = turnViewer.addClient(agentName, filePath, res, allTurns);
