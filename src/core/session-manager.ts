...
import {
  ...
  isRecoverableConversationId,
  recoverPendingApprovalsForAgent,
} from "../tools/letta-api.js";
import {
  installSkillsToAgent,
  prependSkillDirsToPath,
...
function formatSleeptimeStartupOption(
  ...
  if (sleeptime.stepCount !== undefined)
    parts.push(`stepCount=${sleeptime.stepCount}`);
  return parts.length > 0 ? parts.join(", ") : "configured";
}

function toConcreteConversationId(
...
export class SessionManager {
  ...
  private sessionCreationLocks: Map<
    string,
    { promise: Promise<Session>; generation: number }
  > = new Map();
  ...
  private currentCanUseToolByKey = new Map<
    string,
  ...
  syncTodoToolCall(streamMsg: StreamMsg): void {
    ...
    const input =
      streamMsg.toolInput && typeof streamMsg.toolInput === "object"
    ...
    try {
      ...
      if (summary.added > 0 || summary.updated > 0) {
        this.log.info(
          `Synced ${summary.totalIncoming} todo(s) from ${streamMsg.toolName} into heartbeat store (added=${summary.added}, updated=${summary.updated})`,
        );
      }
    } catch (err) {
      this.log.warn(
        "Failed to sync TodoWrite todos:",
        err instanceof Error ? err.message : err,
      );
    }
  }
  ...
  async withSessionTimeout<T>(promise: Promise<T>, label: string): Promise<T> {
   ...
   */
  async ensureSessionForKey(
    key: string,
    bootstrapRetried = false,
  ): Promise<Session> {
    ...
    const promise = this._createSessionForKey(
      key,
      bootstrapRetried,
      generation,
    );
  ...
  private async _createSessionForKey(
    ...
    try {
      ...
      if (
        sessionAgentId &&
        session.agentId &&
        session.agentId !== sessionAgentId
      ) {
        session.close();
        throw new Error(
      ...
      // Sync system prompt on first session init (runs once per bot lifetime).
      // Fire-and-forget: don't block session readiness on the API call.
      if (sessionAgentId) {
        this.maybeSyncSystemPrompt(sessionAgentId);
      }
    } catch (error) {
    ...
    if (!bootstrapRetried && this.store.agentId) {
      try {
        ...
        if (bootstrap.hasPendingApproval) {
          ...
          this.log.warn(
            `Pending approval detected at session startup (key=${key}, conv=${convId}), recovering...`,
          );

          // Try SDK-level recovery first (goes through CLI control protocol)
          const sdkResult = await recoverPendingApprovalsWithSdk(
            session,
            10_000,
          );
          ...
          this.log.warn(
            `SDK recovery did not resolve (${sdkResult.detail ?? "unknown"}), trying API-level recovery...`,
          );
          session.close();
          const result = isRecoverableConversationId(convId)
            ? await recoverOrphanedConversationApproval(
                this.store.agentId,
                convId,
                true,
              )
          ...
          if (result.recovered) {
            this.log.info(
              `Proactive API-level recovery succeeded: ${result.details}`,
            );
          } else {
            this.log.warn(
              `Proactive approval recovery did not find resolvable approvals: ${result.details}`,
            );
          }
          return this._createSessionForKey(key, true, generation);
        }
        ...
        this.log.warn(
          `bootstrapState check failed (key=${key}), continuing:`,
          err instanceof Error ? err.message : err,
        );
      }
    }
    ...
    if ((this.sessionGenerations.get(key) ?? 0) !== generation) {
      this.log.info(
        `Discarding stale session after bootstrapState (key=${key})`,
      );
      session.close();
      return this.ensureSessionForKey(key, bootstrapRetried);
    }
    ...
    if (this.sessions.size >= maxSessions) {
      ...
      for (const [k, ts] of this.sessionLastUsed) {
        ...
        if (this.processingKeys.has(k) || this.sessionCreationLocks.has(k))
          continue;
        if (ts < oldestTime) {
          oldestKey = k;
          oldestTime = ts;
      ...
      if (oldestKey) {
        this.log.info(
          `LRU session eviction: closing session for key="${oldestKey}" (${this.sessions.size} active, max=${maxSessions})`,
        );
        ...
        this.log.debug(
          `LRU session eviction skipped: all ${this.sessions.size} sessions are active/in-flight`,
        );
      }
    }
    ...
    if (session.agentId && session.agentId !== this.store.agentId) {
      ...
      if (convKey && convKey !== "shared") {
        ...
        if (session.conversationId !== existing) {
          ...
          this.log.info(
            `Conversation ID updated (key=${convKey}):`,
            session.conversationId,
          );
        }
  ...
  async runSession(
    ...
    try {
      ...
      if (!retried && isApprovalConflictError(error) && this.store.agentId) {
        this.log.info(
          "CONFLICT detected - attempting SDK approval recovery...",
        );
        ...
        if (sdkResult.recovered) {
          ...
          return this.runSession(message, {
            retried: true,
            canUseTool,
            convKey,
          });
        ...
        if (result.recovered) {
          this.log.info(
            `API-level recovery succeeded (${result.details}), retrying...`,
          );
          return this.runSession(message, {
            retried: true,
            canUseTool,
      ...
      if (this.store.agentId && isConversationMissingError(error)) {
        this.log.warn(
          `Conversation not found (key=${convKey}), creating a new conversation...`,
        );
        this.invalidateSession(convKey);
        if (convKey !== "shared") {
        ...
        try {
          await this.withSessionTimeout(
            session.send(message),
            `Session send retry (key=${convKey})`,
          );
        } catch (retryError) {
          this.invalidateSession(convKey);
          throw retryError;
    ...
    const pendingToolCalls = new Map<
      string,
      { msg: StreamMsg; accumulatedArgs: string }
    >();
    const self = this;
    ...
    function* flushPending(): Generator<StreamMsg> {
      for (const [, pending] of pendingToolCalls) {
        ...
        try {
          toolInput = JSON.parse(pending.accumulatedArgs);
        } catch {
          toolInput = { raw: pending.accumulatedArgs };
        }
      ...
      for await (const raw of session.stream()) {
        ...
        if (msg.type === "tool_call") {
          ...
          if (!id) {
            ...
            const currentPending = lastPendingToolCallId
              ? pendingToolCalls.get(lastPendingToolCallId)
              : null;
            if (
              lastPendingToolCallId &&
              currentPending &&
              (currentPending.msg.toolName || "unknown") ===
          ...
          const incoming =
            (msg as StreamMsg & { rawArguments?: string }).rawArguments || "";
