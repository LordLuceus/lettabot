...
export class LettaBot implements AgentSession {
  ...
  private async prepareMessageForRun(
    ...
    const messageToSend = await buildMultimodalMessage(formattedText, msg, this.log, this.config.inlineImages);
    lap('format message');
  ...
  private async processMessage(msg: InboundMessage, adapter: ChannelAdapter, retried = false): Promise<void> {
    ...
    try {
      ...
      if (now - lastSent >= ERROR_COOLDOWN_MS) {
        ...
        this.log.info(`Suppressed error message to ${msg.chatId} (cooldown: ${Math.round((ERROR_COOLDOWN_MS - (now - lastSent)) / 1000)}s remaining)`);
      }
    } finally {
